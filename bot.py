...
def viral():
    base=["Excavator vs Bulldozer Fight","Big Machine Crash","Transformer Machine Transformation","Human to Robot Transformation","Fighter Jet Dogfight","Space Fight Satellite vs Missile","Powerful Gun vs Tank Fight","Ultra Machine Fight","Biggest Excavator Working","Monster Truck vs Bulldozer","Robot Transformation"]
...
def tts(txt):
    ...
    for i,ch in enumerate([txt_safe[i:i+480] for i in range(0,len(txt_safe),480)][:3]):
        ow=f"v{i}.wav"
        # FIX - piper command with proper escaping
        try:
            ...
            if os.path.exists(ow) and os.path.getsize(ow)>3000:
                print(f"TTS OK {ow} {os.path.getsize(ow)}")
                fl.append(ow)
        except Exception as e: print(f"TTS ERR {e}"); continue

    if not fl:
        print("NO TTS - DUMMY")
        subprocess.run([FF,"-y","-f","lavfi","-i","anullsrc=r=22050:cl=mono","-t","30","dummy.wav"],capture_output=True)
        return ["dummy.wav"]

    # FIX - Try concat, if fail use first file
    try:
        ...
        if os.path.exists("final.wav") and os.path.getsize("final.wav")>5000:
            print(f"FINAL.WAV OK {os.path.getsize('final.wav')}")
            return ["final.wav"]
    except Exception as e: print(f"CONCAT ERR {e}")

    # Fallback - use longest wav
    print(f"FALLBACK TO {fl[0]}")
    return [fl[0]]
...
def dl(q):
    ...
    if not o:
        try:
            r=requests.get(f"https://api.pexels.com/videos/search?query=excavator&per_page=4&size=medium",headers={"Authorization":PEX} if PEX else {},timeout=15).json()
...
script=f"This {raw} in 8K with real machine sound is insane! Watch till the end. The {raw} delivers unbelievable 8K power. Its engine roars like a monster. You can hear the real machine sound. No machine can match its force. It crushes everything in 8K. Who will win this ultra fight? Subscribe for daily 8K fights!"
words=script.split()

# AUDIO - 100% SAFE
af=tts(script)
print(f"USING AUDIO {af}")
audio=AudioFileClip(af[0])
dur=min(T, max(28, audio.duration - 0.15))
...
per=dur/len(words); caps=[]; CAP_W,CAP_H=2000,260; Y_POS=int(H*0.78)
for i,w in enumerate(words):
    if not w.strip(): continue
    cimg=Image.new('RGBA',(CAP_W,CAP_H),(0,0,0,0)); dr=ImageDraw.Draw(cimg,'RGBA')
    try: fnt=ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf",110)
    except: fnt=ImageFont.load_default()
    dr.rectangle((0,0,CAP_W,CAP_H),fill=(0,0,0,180)); dr.text((CAP_W//2,CAP_H//2),w.upper(),fill="#FFEB00",font=fnt,anchor="mm",stroke_width=10,stroke_fill="black")
