"""
anti_bot_detector.py (final) - FIXED
- frame_rule log leak band, only font/colour/fps log
- allp=7 invalid ffmpeg param removed -> exit 8 fix
- No same font, random font colour, randomisation logic, frame variation rule preserved
"""
...
def get_anti_bot_config():
    ...
    print(f"[ANTI-BOT] font={cfg['font']} colour={cfg['color']} fps={cfg['fps']} zoom={cfg['zoom']} id={cfg['id']}")
    return cfg
...
if __name__=="__main__":
    ...
    print(f"VF: {vf[:80]}...")  # FIX: short log only
    print(f"FPS: {fps}")
