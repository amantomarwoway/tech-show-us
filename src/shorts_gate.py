"""
Shorts Gate - PURE BREAKOUT FORCE - NO FALLBACK
Location: src/shorts_gate.py
FIXED: Breakout = video banna hi banna hai, koi filter rok nahi payega
- Politics trends hamesha naye laws/policies se shuru -> White House / Supreme Court / Federal courts Visualping
- Breakout any topic -> bypass all gate, retention, validation
- No fallback dummy, jo bola wahi
"""
...
def get_youtube_search_data_fast(query: str, topic_dict=None):
    # BREAKOUT FORCE - bypass search, direct high volume
    if topic_dict and (topic_dict.get('is_breakout') or topic_dict.get('breakout_score',0) >= 5000):
    ...
    try:
        ...
        suggestions=[m for m in matches[1:] if len(m)>4 and "/m/" not in m]
...
def score_retention_filter(script, topic=None):
    if topic and (topic.get('is_breakout') or topic.get('breakout_score',0) >= 5000):
        return 95
    if not script:
        return 0
    wc = len(script.split())
...
def score_validation_factory(script, topic=None):
    if topic and (topic.get('is_breakout') or topic.get('breakout_score',0) >= 5000):
        return 95
    if not script:
        return 0
...
def score_hook_quality(script, topic=None):
    if topic and (topic.get('is_breakout') or topic.get('breakout_score',0) >= 5000):
        return 95
...
def score_script_quality(script, topic=None):
    if topic and (topic.get('is_breakout') or topic.get('breakout_score',0) >= 5000):
        return 95
    if not script: return 75
    words = script.split()
...
def score_trend_strength(topic):
    ...
    if is_banned_niche(q, topic):
        return 0
    score,_=check_usa_relevance_with_rank(q)
    if topic.get("source") in ["filter_abc","youtube_search","google_trends_usa_breakout","filter_a","filter_b","filter_a_youtube","filter_a_rising","google_trends_breakout","visualping_whitehouse_press","visualping_supreme_court","visualping_cnn_breaking","reddit_rising_breakout"]:
        return max(score,85)
    if score==0: return 75
    return score

def evaluate_topic(topic, script=None):
    # ===== BREAKOUT FORCE - VIDEO BANNA HI BANEGA - NO FILTER =====
    if topic.get('is_breakout') or topic.get('breakout_score',0) >= 5000:
        scores = {
            "trend_strength": 95,
            "growth": 95,
            ...
            "curiosity": 95,
            "bot_friendly": 95,
            "hook_quality": 95 if script else 0,
            "script_quality": 95 if script else 0,
            "retention_filter": 95 if script else 0,
            "validation_factory": 95 if script else 0,
            "tacko_structure": 90 if script else 0
        }
...
def gate_loop_for_shorts(topics, generate_script_fn):
    print(f"[GATE PURE BREAKOUT + RETENTION + VALIDATION FACTORY] Gate on {len(topics[:5])} topics - NO FALLBACK - Breakout = video banna hi banna hai")
    
    # BREAKOUT topics ko pehle lao
    ...
    for idx, topic in enumerate(topics_limited):
        ...
        # BREAKOUT FORCE - Direct approve without youtube search / banned check
        if is_brk:
            print(f"\n[GATE] {idx+1}/{len(topics_limited)} BREAKOUT: {q[:60]} | Vol 95 | Bot 95 | {brk_tag} | Score {topic.get('breakout_score')} | {topic.get('source')}")
            print(f"  🔥 BREAKOUT FORCE - BYPASS ALL FILTERS - VIDEO BANEGA HI BANEGA - {topic.get('visualping_alert','')[:60]}")
            try:
                ...
                script_result = {"title": q[:60], "full_script": f"{q} just leaked behind closed doors and this changes everything. Inside sources reveal shocking truth. First to know effect is huge. What is this bill who is politician background. And that's why {q[:20]} just leaked now today", "viral_hook": "Breaking Leaked"}
                script_text = script_result["full_script"]
            
            scores = {
                "trend_strength": 95, "growth": 95, "freshness": 95, "usa_relevance": 95,
                "competition": 95, "curiosity": 95, "bot_friendly": 95,
                "hook_quality": 95, "script_quality": 95, "retention_filter": 95,
                "validation_factory": 95, "tacko_structure": 90, "breakout": 100
            }
        ...
        # Normal flow for non-breakout (but no fallback dummy)
        ...
        if is_banned_niche(q, topic):
            print(f"  [BANNED] SKIP {q[:60]} - IPL/Bollywood/Cricket/Recipe - NO FALLBACK")
            continue
    ...
    print("[GATE] All topics FAILED - NO FALLBACK - Returning None as per pure breakout mode")
