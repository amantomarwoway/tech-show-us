...
def generate_voice(text, out_path="output/voice.wav"):
    ...
    try:
        ...
        model_path = "piper_model/en_US-amy-medium.onnx"
        # Try multiple possible paths
        for p in [model_path, "models/en_US-amy-medium.onnx", "piper_model/en_US-amy-medium.onnx"]:
            if os.path.exists(p):
                model_path = p
                break
        ...
        with wave.open(temp_raw, 'wb') as wf:
            ...
            for chunk in voice.synthesize(text):
                ...
                wf.writeframes(data)
        print(f"[TTS] Raw generated: {temp_raw}")
    except Exception as e:
        print(f"Piper fail {e}, using espeak-ng fallback")
        # espeak with faster rate 155 ~ 1.15x of normal 135
        safe_text = text.replace('"','').replace("'","")[:500]
        os.system(f'espeak-ng -v en-us "{safe_text}" -s 165 -p 55 --stdout > {temp_raw}')
    ...
    print(f"[TTS FINAL] {out_path} - Speed {TTS_SPEED}X, Punch {PUNCH_VOLUME} first 1s, Vol mod every {VOL_MODULATION_EVERY_SEC}s {int(VOL_MODULATION_PERCENT*100)}% + bass drop")
    return out_path
